...
class CF_BL:
    ...
    def __init__(self, K, p, c,t, rf, M_e, N_e, M_b, N_b, theta_e, theta_b, precision_e, precision_b):
        ...
        self.probability_e_m = lambda m: betabinom.pmf(m, M_e, self.a_e, self.b_e)
        
        self.probability_e_D = lambda x, m: betabinom.pmf(x, self.N_e, self.a_e + m, self.b_e + self.M_e - m )
        
        # Bank's belief
        self.probability_b_m = lambda m: betabinom.pmf(m, M_b, self.a_b, self.b_b)
    ...
    def CF(self, m):
        ...
        if np.isnan(r):
            ...
            return np.maximum(self.spot_market_revenue_knowing_m(m) - self.c * self.K - (production_cost - self.p* (1-self.t) * m) * r, 0)
...
class BL:
    ...
    def mean_profit(self):
        ...
        if np.isnan(r):
            return 0
        return np.maximum(self.total_market_revenue() - self.c * self.K * (1 + r), 0)
...
class oracle_bl:
    ...
    def __init__(self, K, p, c, rf, M_e, N_e, M_b, N_b, theta_oracle, theta_b, precision_b):
        self.K = K          # Inventory
        self.p = p          # Price
        self.c = c          # Cost 
        self.rf = rf        # Risk-free rate
        ...
        self.theta__oracle = theta_oracle  # Entrepreneur's belief parameter
        self.theta_b = theta_b  # Bank's belief parameter
        self.precision_b = precision_b


        # oracle's belief
        self.probability_oracle= lambda m: binom.pmf(m, self.M_e + self.N_e, self.theta__oracle)
        ...
        # Bank's belief
        self.probability_b = lambda m: binom.pmf(m, self.M_b + self.N_b , self.a_b, self.b_b)
    ...
    def total_market_revenue(self):
        ...
        integrand = lambda x: np.minimum(x, self.K)*self.probability_oracle(x)
...
class oracle_CF_BL:
    ...
    def __init__(self, K, p, c,t, rf, M_e, N_e, M_b, N_b, theta_oracle, theta_b, precision_b):
        self.K = K          # Inventory
        self.p = p          # Price
        self.c = c          # Cost 
        self.t = t
        ...
        self.theta_oracle = theta_oracle  # Entrepreneur's belief parameter
        self.theta_b = theta_b  # Bank's belief parameter
        ...
        # Entrepreneur's belief
        self.probability_oracle_m = lambda m: binom.pmf(m, M_e, self.theta_oracle )
        
        self.probability_oracle_D = lambda x, m: binom.pmf(x, self.N_e, self.theta_oracle)
    ...
    def spot_market_revenue_knowing_m(self, m):
        """Calculate the spot market revenue given m."""
        integrand = lambda x: np.minimum(x, self.K) * self.probability_oracle_D(x, m)
        return np.sum(integrand(np.arange(0, self.N_e + 1))) * self.p +   m * self.p * (1-self.t)
    ...
    def mean_profit(self):
        """Calculate the expected profit of the entrepreneur."""
        integrand_cf = lambda m: self.CF(m) * self.probability_oracle_m(m)
        return np.sum([integrand_cf(m) for m in range(0, self.M_e + 1)])
