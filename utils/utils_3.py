...
class CF_BL:
    ...
    def spot_market_revenue_knowing_m(self, m):
        ...
        integrand = lambda x: np.minimum(x, self.K- m) * self.probability_e_D(x, m)
    ...
    def profit_banque(self, r, m):
        ...
        S = self.c * self.K  - self.p * (1-self.t) * m
        integrand_b = lambda x: np.minimum(S * (1 + r), np.minimum(x * self.p, (self.K - m)* self.p)) * self.probability_b_D(x, m)
